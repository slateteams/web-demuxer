const getOrCreateWorkerFile = (file) => {
  if (file instanceof File) {
    return new WorkerFile(file);
  } else {
    return new WorkerRemoteFile(file);
  }
}

const getFilename = (file) => {
  if (file instanceof WorkerFile) {
    return file.fileName;
  } else {
    return `remote-${encodeURIComponent(file).replace(/[^a-zA-Z0-9]/g, "-")}`;
  }
}
...
class WorkerRemoteFile {
  constructor(url) {
    this.url = url;
    this.mountPoint = "/remote";
    this.fileName = `remote-${encodeURIComponent(url).replace(/[^a-zA-Z0-9]/g, "-")}`;
    this.filePath = `/remote/${this.fileName}`;
  }

  mount() {
    try {
      FS.readdir(this.mountPoint);
    } catch(e) {
      FS.mkdir(this.mountPoint);
    }
  }
...
async function fetchFile(url) {
  const workerFile = new WorkerRemoteFile(url);
  await workerFile.mount();

  // stream download video
  const stream = await fetch(url);
  const reader = stream.body.getReader();

  const file = FS.open(workerFile.filePath, "w");
  let offset = 0;
  while(true) {
    const { done, value } = await reader.read();
    if (done) break;
    // write value to file
    FS.write(file, value, 0, value.length, offset);
    offset += value.length;
  }
  FS.close(file);
}
...
function getAVStream(file, type = 0, streamIndex = -1) {
  const workerFile = getOrCreateWorkerFile(file);
