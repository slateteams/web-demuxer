...
function extractStream(file, type, streamIndex) {
  const workerFile = new WorkerFile(file);

  workerFile.mount();

  try {
    console.log('extract_stream', workerFile.filePath, type, streamIndex);
    const path = Module.extract_stream(workerFile.filePath, type, streamIndex);
    console.log('extract_stream done', path);
    console.log('reading...');
    const buffer = FS.readFile(path).buffer;
    const uint8Array = new Uint8Array(buffer);

    return uint8Array;
  } catch(e) {
    throw new Error("extract_stream failed: " + e.message);
  } finally {
    workerFile.unmount();
  }
}
...
Module.extractStream = extractStream;
